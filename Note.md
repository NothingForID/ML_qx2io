...
- 标签 Label  
	模型需要重点预测的“目标变量”（常是表格的最后一列）。  
	所谓预测，本质是机器通过学习特征用以推算数据中的标签  
...
- 运行机制  
	在环境中设置智能体(Agent)，根据智能体采取动作的结果好坏，实时反馈奖励(Reward)或惩罚(Penalty)。  
	最终形成在长期过程中取得最大累计奖励的策略。  
- 应用场景  
	1. 棋牌或电子游戏  
	2. 机器人姿态控制  
	3. 工业机械臂调度  
...
尽管特征($x^2,x^3$)出现高阶非线性项，  
但是模型对参数($w_1, w_2, \ldots, w_d, b$)仍是线性的  
...
$$
\text{输入} \overset{\text{线性+激活}}{\longrightarrow} 隐藏层_{1} \overset{\text{线性+激活}}{\longrightarrow} 隐藏层_{2} \rightarrow \ldots \rightarrow \text{输出}
$$  
...
1. **Age (年龄)**:  
	...
	- 但在 60-80 岁之间有很多离群点  
  	这说明船上有不少高龄乘客，虽然人数少，但他们是真实存在的，不是数据错误  
1. **Fare (票价)**:  
	- **极度右偏**  
		绝大多数人的票价非常低（箱体被压扁在底部，接近 0-50 美元）  
	- 但是有极少数人的票价高得离谱，超过 500 美元！这些可能是一等舱乘客  
	- **对模型的影响**  
		极端数值会让模型难以训练，若不处理（比如取对数 `log` 或分箱），模型则可能会过度关注这几个高价票乘客，而忽略普通大众  
2. **SibSp (兄弟姐妹/配偶数)**:  
	- 大部分人都是 0 或 1，独自旅行或夫妻同行  
	- 但也有一些大家庭，带着 3、4、5 甚至 8 个亲属，这也是真实的异常值  

使用统计学方法（IQR）识别并处理票价（Fare）中的极端异常值，防止它们干扰机器学习模型的训练。  
...
这三种编码方式的核心区别在于是否保留了类别的顺序信息以及是否引入了虚假的数值大小关系。  
选择哪种方式，取决于你的数据特征类型和你使用的模型算法。  
...
`y_pred`是模型对测试集的预测结果，`y_test`是真实值  
评估要解决的问题:  
1. 计算二者差异  
2. 如何量化差异  
3. 区分二者差异  
...
$$
MAE = \frac{1}{n} \sum \limits _{i=1} ^{n} \left| y_i - \hat{y_i} \right|
$$
...
若换一个 `random_state`，R² 可能从 0.57 变成 0.60 或 0.55。  
如何判断 0.57 是否反映模型的真实水平，而非单次划分运气所致？
...
- **degree=1**:  
	线性模型，无法捕捉二次关系，导致欠拟合  
- **degree=2**:  
	二次多项式，与真实数据生成过程匹配，良好拟合  
- **degree=15**:  
	高次多项式，过度拟合训练数据中的噪声，导致过拟合  
...
| **诊断结果**                      | **主要表现**                        | **核心问题**    | **解决方案**                                                                                                    |
|:-----------------------------:| ------------------------------- | ----------- | ------------------------------------------------------------------------------------------------------------ |
| **欠拟合**<br>Underfitting | 训练集得分：**差**<br>测试集得分：**差**  | 模型表达能力不足    | ➕ **做加法：**<br>1. 增加特征（特征工程/多项式特征）<br>2. 增加训练轮数<br>3. 换用更复杂的模型                                    |
| **过拟合**<br>Overfitting  | 训练集得分：**极好**<br>测试集得分：**差** | 模型过于敏感、死记硬背 | ➖ **做减法：**<br>1. 引入正则化<br>2. 提供更多的训练数据<br>3. 简化模型结构（如剪枝、减少特征维度）<br>4. 提前停止训练（Early Stopping） |
...
$$
\begin{cases} 
L'(p) < 0 , & p\in(\cfrac45, 1) \\ 
L'(p) = 0, & p = 0, \cfrac45, 1 \\ 
L'(p) > 0, & p \in (0,\cfrac45) 
\end{cases}
$$ 
...
| **真实标签** | **损失项**           | **含义**                                 |
| -------- | ----------------- | -------------------------------------- |
| $y = 1$  | $-log(\hat{y})$   | 预测概率越接近 1，损失越小；<br>接近 0 时损失 $\to + \infty$ |
| $y = 0$  | $-log(1-\hat{y})$ | 预测概率越接近 0，损失越小；<br>接近 1 时损失 $\to + \infty$ |
...
从二分类扩展到多分类时，需要 Softmax 回归 (也叫多项逻辑回归，Multinomial Logistic Regression)  
Softmax 回归能一次性输出所有类别的概率。  
...
- 花瓣中等 → 多为变色鸢尾
- 花瓣最长 → 多为维吉尼亚鸢尾
- 花瓣长度 >4.75 且 1.6 < 宽度 <1.75 时变色鸢尾与维吉尼亚有重叠
...
2. 后剪枝 Post-pruning  
	...
	如果剪掉某个底部分支并不会导致准确率明显下降，甚至能提高在测试集上的表现，那就应该把它剪掉。  
...
2. 特征随机 Random Subspace  
	...
	而是随机抽取其中一个子集 (通常取 $\sqrt{d}$ 个特征， $d$ 为总特征数)，  
	仅在这些候选特征中寻找最优分裂点  
...
$$
P(未被抽中) = (1 - \frac{1}{N})^N \overset{N \to \infty}{\longrightarrow} \frac{1}{e} \approx 36.8 \text{pct}
$$
...
3. 集成输出  
	...
	- 分类任务采用 **多数表决** Majority Voting  
		${\large \hat{y} = \arg \max \limits_{c} \sum\limits_{t=1}^T \mathbb{I} \big[h_t(x) = c\big]}$  
		$\hat{y}$ : 每棵树输出一个标签   
		$\arg\max\limits_c$ : 所有可能的类别中，使后续表达式最大化的类别 $c$  
		$\sum \limits _{t=1} ^T$ : 从 $t = 1$ 到 $T$ 求和  
		$h_t(x)$ : 第 $t$ 个基础学习器(分类器) 对输入 $x$ 的预测输出  
		$\mathbb{I} \big[ \rm condition \big]$ : 指示函数 Indicator Function  
		- 若condition为真则 $\mathbb{I} \big[ \rm condition \big] = 1$ 
		- 若condition为假则 $\mathbb{I} \big[ \rm condition \big] = 0$   
	
	- 回归任务采用 **简单平均** Averaging  
		${\large \hat{y} = \cfrac {1}{T} \sum\limits_{t=1}^T h_t(x)}$  
		$\hat{y}$ : 每棵树输出一个数值  
...
他们目标相同，都要把多个弱模型组成一个强模型，但是实现路径不同
...
## K-Means 核心思想
K-Means 
- 是经典聚类算法，将一批无标签数据自动划分为K个簇  
  - K : 预先指定的簇数
  - Means : 均值，即每个簇的中心位置由该簇内所有样本的均值来表示
- 目标是找到K个具有代表性的中心点，使每个样本归属于离自己最近的中心，并尽量减小样本到中心的距离
...
- $\cfrac{1}{n-1}$ : 贝塞尔修正 Bessel's correction
  
> [!TIP]  
> 通过样本数据推断总体特性时，协方差矩阵的无偏估计需要贝塞尔修正  
> 简单的 $\cfrac1n$ 会低估方差
> - $\frac1n$ : 有偏估计
> - $\frac{1}{n-1}$ : 无偏估计
...
1. 线性假设局限  
	PCA 假设数据的主成分是线性的。  
	对于非线性结构数据，传统 PCA 效果不佳，需要 Kernel PCA 或 t-SNE、UMAP 等非线性降维算法。  
2. 丧失可解释性  
